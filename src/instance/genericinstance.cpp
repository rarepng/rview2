...
void genericinstance::playanimation(int animNum, float speedDivider, float blendFactor, replaydirection direction) {
	...
	if(mmodelsettings.animloop)
		mmodelsettings.msanimtimepos = std::fmod((currentTime-mmodelsettings.animstart) / 1000.0 * speedDivider, manimclips.at(animNum)->getEndTime());
	else
...
void genericinstance::playanimation(int sourceAnimNumber, int destAnimNumber, float speedDivider, float blendFactor, replaydirection direction) {
	double currentTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
...
void genericinstance::crossblendanimationframe(int sourceAnimNumber, int destAnimNumber,
